...
void led_wr_str(char lin, char col, char str[], char leng) {

    static char buffer[32];

    for (uint8_t i = 0; i < leng; i++) {
        buffer[lin * 16 + col + i] = str[i];
    }

    for (uint8_t i = 0; i < 32; i++) {
        uart_tx_bit_bang(buffer[i]);
    }
